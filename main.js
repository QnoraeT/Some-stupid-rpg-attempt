...
function verifyElementChecks(elem) {
    for (let i = 0; i < ELEMENT_LIST.length; ++i) {
        console.info("Check: " + [ELEMENT_LIST[i]] + " was good.  (DMG: x" + checkElementEffective([ELEMENT_LIST[i]], elem) + ")");
    }
}
...
function allInQueue() {
    ...
    for (let i = 0; i < damageList.length; ++attempts) {
        if (attempts > 10000) {
            throw new Error(`Hey! Something went wrong! I can't process this! (attempted damageQueue ${attempts} times, damageQueue: ${damageList})`);
        }
        if (damageList[i] === "Damage") {
            dI = damageList[i + 10];
            let victim = damageList[i + 2];

            if (!damageList[i + 9]) {people[victim].extraVar.hit = damageList[i + 1];}
            ...
            if (crit > 0) {
                console.log(CRITICAL_HIT_NAMES[damage] + " Hit!");
            ...
            try {
                ...
                // * make this return trueDamage or 'NoDIN'!!
            } catch {
                console.log(`${victim} doesn't have a special damage script! preceding as normal...`)
                people[victim].health = people[victim].health - trueDamage;
                people[victim].hitTimer = 0;
                if (people[victim].health <= 0) {console.log("Mortal! [" + format(-1 * people[victim].health, 3, 1000000) + "] HP overkill");}
                console.log(format(trueDamage, 3, 1000000) + " damage");
            }

            try {
                peopleObj[victim].hitScript(trueDamage);
            } catch {
                console.warning(`${victim} does not have a hit script! Please add a hit script to ${victim} in objects.js!`);
            }
            if (dI[0] != "NoDIN") {
                if (dI[0] === "") {
                    dI[0] = format(trueDamage, 3, 1000);
                }
            }
            dI[6] = people[victim].xPosition + people[victim].xPosHP;
            dI[7] = people[victim].yPosition + people[victim].yPosHP;
            if (crit > 0) {
                let prev = [dI[6], dI[7], dI[2]]
                dI[6] += dI[1] / 12;
                dI[7] -= dI[1] / 12;
                dI[2] = mixColor(dI[2], CRIT_COLOR[crit], "Linear", 0.5)
                makeDamageIndicator(...dI);
                dI[6] = prev[0]
                dI[7] = prev[1]
                dI[2] = prev[2]
            }
            ...
            for (let j = damageList[i + 3].length - 1; j >= 0; --j) {
                ...
                switch (j) {
                    case 0:
                        ...
                        if (people[damageList[i + 2]].health >= people[damageList[i + 2]].maxHealth) { people[damageList[i + 2]].health = people[damageList[i + 2]].maxHealth; }
                        break;
                    case 1:
                        people[damageList[i + 2]].mana = people[damageList[i + 2]].mana + healing;
                        if (people[damageList[i + 2]].mana >= people[damageList[i + 2]].maxMana) { people[damageList[i + 2]].mana = people[damageList[i + 2]].maxMana; }
                        break;
                    case 2:
            ...
            if (people[pA].sEffects.includes(effectType)) {
                ...
                switch (damageList[i + 6]) {
                    ...
                    case 6:
                        ...
                        break;
                    default:
                        throw new Error("What do you want me to do here?? " + people[pA].name + " already has the effect " + EFFECT_LIST[effectType]) + ", but you are not clear what you want me to do! (Type: " + damageList[i + 6] + ")";
                }
            } else {
                people[pA].sEffects.push(effectType);
                people[pA].sDuration.push(duration);
...
function updateVisuals() {
    ...
    ShakeX = Math.floor(Shake) * lerp(((Time - shakeRandomInterval) / 0.02) + 1, shakeRandomListX[0], shakeRandomListX[1], "Sine");
    ShakeY = Math.floor(Shake) * lerp(((Time - shakeRandomInterval) / 0.02) + 1, shakeRandomListY[0], shakeRandomListY[1], "Sine");
    let dlastHP;
    let dcurrHP;
    for (let i = 0; i < peopleNames.length; ++i) {
        ...
        characters[i].style.transform = "translate(-50%, -50%) scale(" + (width * people[peopleNames[i]].flip[0]) + "," + (width * people[peopleNames[i]].flip[1]) + ") rotate(" + (people[peopleNames[i]].direction - 90) + "deg)";
        let FIL =
        ...
        if (Time > lastTurn) {
            ...
            if (currentState.ended === 0) {
                if (people[peopleNames[turnID]].alive === true) {
                    ...
                    people[peopleNames[turnID]].tickStatusEffects();
                } else {
